...
std::vector<VkSemaphore> g_RenderCompleteSem;
std::vector<VkSemaphore> g_PresentCompleteSem;
std::vector<VkFence> g_Fence;
...
glm::vec4 materialDiffuse = glm::vec4(0.5f, 0.2f, 0.7f, 1.0f);
glm::vec4 materialSpecular = glm::vec4(0.7f, 0.7f, 0.7f, 1.0f);
...
int main()
{
	...
	windowAttribute.colormap = XCreateColormap(display, 
						...
						visualInfo->visual,
						AllocNone);
	windowAttribute.event_mask = ExposureMask | KeyPressMask| StructureNotifyMask | FocusChangeMask | PointerMotionMask | ButtonMotionMask | ButtonPressMask | ButtonReleaseMask;
	...
	float mouse_x = 0.0f;
	float mouse_y = 0.0f;
	int currentbutton = MOUSE_BUTTON_NONE;
    // Main loop
    while (b_CloseLoop == False) 
	{
		...
		while (XPending(display)) // XPending means Peak Message
		{
			...
			case MotionNotify:
				...
				camera.mouseInputs(currentbutton, MOUSE_ACTION_MOVE, mouse_x, mouse_y);
				break;
			case ButtonPress:
				if (event.xbutton.button == Button1) { // left click
					camera.mouseInputs(MOUSE_BUTTON_LEFT, MOUSE_ACTION_PRESS, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_LEFT;
				} else if(event.xbutton.button == Button2) { // middle click
					camera.mouseInputs(MOUSE_BUTTON_MIDDLE, MOUSE_ACTION_PRESS, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_MIDDLE;
				} else if(event.xbutton.button == Button3) { // right click
					camera.mouseInputs(MOUSE_BUTTON_RIGHT, MOUSE_ACTION_PRESS, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_RIGHT;
				}
				break;
			case ButtonRelease:
				if (event.xbutton.button == Button1) { // left click
					camera.mouseInputs(MOUSE_BUTTON_LEFT, MOUSE_ACTION_RELEASE, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_NONE;
				} else if(event.xbutton.button == Button2) { // middle click
					camera.mouseInputs(MOUSE_BUTTON_MIDDLE, MOUSE_ACTION_RELEASE, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_NONE;
				} else if(event.xbutton.button == Button3) { // right click
					camera.mouseInputs(MOUSE_BUTTON_RIGHT, MOUSE_ACTION_RELEASE, mouse_x, mouse_y);
					currentbutton = MOUSE_BUTTON_NONE;
				}
				break;
...
void createSwapChain(uint32_t width, uint32_t height)
{
	...
	std::cout << "CommandBufferCount : " << g_CommandBufferCount << "\n";
...
void initializeQueue()
{
	...
	g_RenderCompleteSem.resize(MAX_FRAMES_IN_FLIGHT);
	g_PresentCompleteSem.resize(MAX_FRAMES_IN_FLIGHT);
	g_Fence.resize(MAX_FRAMES_IN_FLIGHT);

	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		// create render complete semaphore
		if (vkCreateSemaphore(g_LogicalDevice, &semaphoreCreateInfo, nullptr, &g_RenderCompleteSem[i]) != VK_SUCCESS)
		{
		...
		if (vkCreateSemaphore(g_LogicalDevice, &semaphoreCreateInfo, nullptr, &g_PresentCompleteSem[i]) != VK_SUCCESS)
		{
		...
		// create fence
		if (vkCreateFence(g_LogicalDevice, &fenceCreateInfo, nullptr, &g_Fence[i]) != VK_SUCCESS)
		{
...
void resize(uint32_t width, uint32_t height)
{
	...
	camera.updateResolution((float)winWidth, (float)winHeight);
}
...
void render()
{
	...
	// fence
	vkWaitForFences(g_LogicalDevice, 1, &g_Fence[currentFrame], VK_TRUE, UINT64_MAX);
	vkResetFences(g_LogicalDevice, 1, &g_Fence[currentFrame]);

	// acquire the next image from the swapchain
	uint32_t imageIndex = -1;
	VkResult result = VK_SUCCESS;
	result = vkAcquireNextImageKHR(g_LogicalDevice, g_SwapChain, UINT64_MAX, g_PresentCompleteSem[currentFrame], nullptr, &imageIndex);
	if (result != VK_SUCCESS)
	{
	...
	VkSubmitInfo submitInfo =
	{
		...
		.pWaitSemaphores = &g_PresentCompleteSem[currentFrame],
		.pWaitDstStageMask = &waitFlags,
		.commandBufferCount = 1,
		.pCommandBuffers = &g_CommandBuffer.at(imageIndex),
		.signalSemaphoreCount = 1,
		.pSignalSemaphores = &g_RenderCompleteSem[currentFrame]
	};

	if (vkQueueSubmit(g_Queue, 1, &submitInfo, g_Fence[currentFrame]) != VK_SUCCESS)
	{
	...
	VkPresentInfoKHR presentInfo = 
	{
		...
		.pNext = nullptr,
		.waitSemaphoreCount = 1,
		.pWaitSemaphores = &g_RenderCompleteSem[currentFrame],
		.swapchainCount = 1,
...
void updateUniformBuffer(uint32_t curImage)
{
	...
	if (bLight == true)
	{
		...
		ubo.lightingEnabledUniform = bLight;
	}
	else
	{
		ubo.lightingEnabledUniform = bLight;
...
void uninitialize()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) 
	{
		vkDestroySemaphore(g_LogicalDevice, g_RenderCompleteSem[i], nullptr);
		g_RenderCompleteSem[i] = VK_NULL_HANDLE;
		// if (g_PresentCompleteSem != VK_NULL_HANDLE)

		vkDestroySemaphore(g_LogicalDevice, g_PresentCompleteSem[i], nullptr);
		g_PresentCompleteSem[i] = VK_NULL_HANDLE;

		vkDestroyFence(g_LogicalDevice, g_Fence[i], nullptr);
		g_Fence[i] = VK_NULL_HANDLE;
	}
