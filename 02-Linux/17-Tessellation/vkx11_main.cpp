...
int main()
{
	...
	XVisualInfo v_Info;
	XMatchVisualInfo(display, screen, XDefaultDepth(display, screen), TrueColor, &v_Info);
	visualInfo = &v_Info;
...
void createSwapChain(uint32_t width, uint32_t height)
{
	...
	{
		// std::cout << "\n######### Physical Device Surface Present Modes " << presentModes.size() <<" #########\n";
...
void createGraphicsPipeline()
{
	...
	std::array<VkVertexInputAttributeDescription, 1> getAttributeDescriptions(void);
	VkShaderModule createShaderModule(const std::vector<char> &shaderCode);

	// code
	auto vertexShaderCode = readFile("shaders/tessellation.vert.spv");
	auto fragmentShaderCode = readFile("shaders/tessellation.frag.spv");
	auto tessellationControlShaderCode = readFile("shaders/tessellationControl.tesc.spv");
	auto tessellationEvalShaderCode = readFile("shaders/tessellationEval.tese.spv");

	if (vertexShaderCode.empty() || fragmentShaderCode.empty() || tessellationControlShaderCode.empty() || tessellationEvalShaderCode.empty())
	{
	...
	std::cout << "Tessellation Control Shader Code Size: " << tessellationControlShaderCode.size() << "\n";
	std::cout << "Tessellation Evaluation Shader Code Size: " << tessellationEvalShaderCode.size() << "\n";
	...
	{
		std::cerr << "Failed to create tessellation control shader module\n";
		return;
	}

	// Tess Evaluation shader
	VkShaderModule tessellationEvalShaderModule = createShaderModule(tessellationEvalShaderCode);
	if (tessellationEvalShaderModule == VK_NULL_HANDLE)
	{
		std::cerr << "Failed to create tessellation evaluation shader module\n";
		return;
	}
	...
	// shader stage for fragment shader
	...
	// shader stage for Tess Control shader
	VkPipelineShaderStageCreateInfo tessellationControlShaderStageInfo =
	{
		.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
		.stage = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
		.module = tessellationControlShaderModule,
		.pName = "main"
	};

	// shader stage for Tess Evaluation shader
	VkPipelineShaderStageCreateInfo tessellationEvalShaderStageInfo =
	{
		.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
		.stage = VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT,
		.module = tessellationEvalShaderModule,
		.pName = "main"
	};

	VkPipelineShaderStageCreateInfo shaderStages[] = {vertexShaderStageInfo, tessellationControlShaderStageInfo, tessellationEvalShaderStageInfo, fragmentShaderStageInfo};

	
	// ===================================================================================================
	VkVertexInputBindingDescription bindingDescription = getBindingDescription();
	std::array<VkVertexInputAttributeDescription, 1> attributeDescriptions = getAttributeDescriptions();
	...
	// Tessellation input stage
	VkPipelineTessellationStateCreateInfo tessellationInputInfo =
	{
	...
	VkGraphicsPipelineCreateInfo pipelineCreateInfo =
	{
		...
		.pNext = nullptr,
		.flags = 0,
		.stageCount = 4, // vertex and fragment shader states
		.pStages = shaderStages,
		.pVertexInputState = &vertexInputInfo,
		.pInputAssemblyState = &inputAssembly,
		.pTessellationState = &tessellationInputInfo,  // if tessellation is used
	...
	vkDestroyShaderModule(g_LogicalDevice, tessellationControlShaderModule, nullptr);
	vkDestroyShaderModule(g_LogicalDevice, tessellationEvalShaderModule, nullptr);
}
...
void createDescriptorSetLayout()
{
	...
	/* Add the layout bindings like UBO's , Samplers */
	// std::array<VkDescriptorSetLayoutBinding, 2> shaderBindings = {uboLayoutBinding, samplerLayoutBinding};
	std::array<VkDescriptorSetLayoutBinding, 1> shaderBindings = {uboLayoutBinding};
...
void updateUniformBuffer(uint32_t curImage)
{
	...
	ubo.modelMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	ubo.numberOfStripes = 1;
	ubo.uiNumberOfSegments = g_NumSegments;
