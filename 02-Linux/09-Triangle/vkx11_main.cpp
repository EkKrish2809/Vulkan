...
void createVertexBuffer()
{
	...
	// VkBuffer createVulkanBuffers(VkDeviceSize size, VkBufferCreateInfo bufferCreateInfo, VkBufferUsageFlags usage, VkSharingMode sharingMode);
	...
	VkBufferCreateInfo bufferInfo = 
	{
		...
		nullptr,                               									// pNext
		0,                                     									// flags
		sizeof(vertices[0]) * vertices.size(),                                     						// size can be a parameter
		VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,    // usage can be decided based on usecase ((as a parameter))
		VK_SHARING_MODE_EXCLUSIVE,             												// sharingMode
		0,                                     									// queueFamilyIndexCount
		nullptr   
	};
	if (vkCreateBuffer(g_LogicalDevice, &bufferInfo, nullptr, &g_VertexBuffer) != VK_SUCCESS)
	{
		std::cerr << "Failed to create Vulkan buffer\n";
		return ;
	}
	// g_VertexBuffer = createVulkanBuffers(sizeof(vertices[0]) * vertices.size(), bufferInfo, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE);
	// if (g_VertexBuffer == VK_NULL_HANDLE)
	// {
	...
	// find the memory requirements for the above created Vertex Buffer
	VkMemoryRequirements memoryRequirements;
	vkGetBufferMemoryRequirements(g_LogicalDevice, g_VertexBuffer, &memoryRequirements);

	// do the memory allocation for Vertex Buffer
	VkMemoryAllocateInfo memoryAllocateInfo =
	{
		.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
		.allocationSize = memoryRequirements.size,
		.memoryTypeIndex = findMemoryTypes(memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
	};

	if (vkAllocateMemory(g_LogicalDevice, &memoryAllocateInfo, nullptr, &g_VertexBufferMemory) != VK_SUCCESS)
	{
	...
	// if above memory allocation is successful, we should now bind the memory with the buffer
	if (vkBindBufferMemory(g_LogicalDevice, g_VertexBuffer, g_VertexBufferMemory, 0) != VK_SUCCESS)
	{
		std::cerr << "Failed to bind the VertexBuffer to the VertexBufferMemory\n";
		return;
	}

	// filling the vertex buffer
	void *data;
	vkMapMemory(g_LogicalDevice, g_VertexBufferMemory, 0, bufferInfo.size, 0, &data);
...
VkBuffer createVulkanBuffers(VkDeviceSize size, VkBufferCreateInfo bufferCreateInfo, VkBufferUsageFlags usage, VkSharingMode sharingMode = VK_SHARING_MODE_EXCLUSIVE)
{
